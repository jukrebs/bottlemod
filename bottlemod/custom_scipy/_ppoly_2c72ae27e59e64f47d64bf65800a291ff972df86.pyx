...
# modified version for bottlemod
...
def evaluate(double_or_complex[:,:,::1] c,
    ...
    for ip in range(len(xp)):
        ...
        for jp in range(c.shape[2]):
            out[ip, jp] = evaluate_poly1(xval, c, interval, jp, dx)
    ...
    for ip in range(1, len(x)-1):
        ...
        for jp in range(c.shape[2]):
            ...
            for dx in range(order, -1, -1):
                ...
                res = evaluate_poly1(xval, c, interval, jp, dx)
    ...
    for jp in range(c.shape[2]):
        ...
        for interval in range(start_interval, end_interval+1):
            ...
            if interval == end_interval:
                vb = evaluate_poly1(b, c, interval, jp, -1)
            else:
                vb = evaluate_poly1(x[interval+1], c, interval, jp, -1)

            # local antiderivative, start point
            if interval == start_interval:
                va = evaluate_poly1(a, c, interval, jp, -1)
            else:
                va = evaluate_poly1(x[interval], c, interval, jp, -1)
        ...
        for jp in range(c.shape[2]):
            ...
            for interval in range(c.shape[1]):
                ...
                if interval > 0 and report_discont:
                    va = evaluate_poly1(x[interval], c, interval-1, jp, 0) - y
                    vb = evaluate_poly1(x[interval-1], c, interval, jp, 0) - y
