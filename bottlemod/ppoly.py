...
class PPoly(scipy.interpolate.PPoly):
    ...
    def __call__(self, other, nu=0, extrapolate=None):
        if isinstance(other, PPoly):
            ...
            for x in list(heapq.merge(outerchanges, other.x))[0:-1]:
                if x != lastx:
                    newpoly = numpy.array(PPoly.__insertPoly(x, self, otherpows))
                    if len(newpoly) != len(lastpoly) or numpy.any(numpy.not_equal(newpoly, lastpoly)):
                        lastpoly = newpoly
                        newx.append(x)
                        newc = [
                            c1 + c2
            ...
            for x in list(heapq.merge(self.x, other.x))[0:-1]:
                if x != lastx:
                    newpoly = numpy.array(PPoly.__insertPolyOld(x, self, otherpows))
                    if len(newpoly) != len(lastpoly) or numpy.any(numpy.not_equal(newpoly, lastpoly)):
                        lastpoly = newpoly
                        newx.append(x)
                        newc = [
            ...
            return super().__call__(other, nu=nu, extrapolate=extrapolate)
    ...
    def roots(self, discontinuity=True, extrapolate=None):
        result = list(super().roots(False, extrapolate=extrapolate))
        if discontinuity:
            lastsector = self[self.x[0]]
            for bp in self.x[1:-1]:
                thissector = self[bp]
                before = float(numpy.asarray(lastsector(bp)).item())
                after = float(numpy.asarray(thissector(bp)).item())
